...
def test_new_email_login():
    print("📧 새 이메일로 로그인 테스트")
    print("=" * 50)
    
    # 새 이메일로 로그인 데이터
    login_data = {
        "email": "stttest01@g.com",
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ 새 이메일로 로그인 성공!")
            print(f"응답 데이터: {json.dumps(result, indent=2, ensure_ascii=False)}")
            if 'access_token' in result:
                print(f"🎉 JWT 토큰 발급됨: {result['access_token'][:50]}...")
            print(f"\n=== 사용자 정보 확인 ===")
            print(f"  사용자 ID: {result['user']['user_id']}")
            print(f"  이메일: {result['user']['email']}")
            print(f"  사용자명: {result['user']['name']}")
        else:
            print(f"❌ 로그인 실패")
            try:
