...
def create_api_key():
    ...
    token_id = f"korean_test_token_{random.randint(1000, 9999)}"
    
    response = requests.post(
        f"{BASE_URL}/tokens/{token_id}",
        headers=headers,
        params={"description": "한국어 음성 테스트용 토큰"}
    )
...
def test_korean_voice():
    """
    한국어 음성 파일로 /transcribe/protected/ 엔드포인트 테스트
    """
    print("🔑 API 키 생성 중...")
    ...
    # 한국어 음성 파일로 테스트
    audio_file = "korean_voice_test.wav"
    ...
    try:
        ...
        if result:
            ...
            if result.transcribed_text and result.transcribed_text.strip():
                print("\n🎉 한국어 transcribed_text가 올바르게 저장되었습니다!")
            else:
